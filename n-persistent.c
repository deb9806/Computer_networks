...
{
	int i,n1,n2;
	int m1=-1,m2=-1,n,e1=1,e2=1; //acknowledgement identifier
	...
	label1 :
		 l=l+1;
		 if(m1==1) //waitiing stage for sender b to get free
		 {
			 printf("\n\n--WAITING FOR CHANNEL TO GET FREE (RANDOM_WAIT)-- (B)");
			 printf("\n--INCREASING IN BACKOFF TIME--");
			 delay(4000 * bk(e1));
			 e1++;
		 }
		 else if(m2==1) //waitiing stage for sender a to get free
		 {
			 printf("\n\n--WAITING FOR CHANNEL TO GET FREE (RANDOM_WAIT)-- (A)");
			 printf("\n--INCREASING IN BACKOFF TIME--");
			 delay(4000 * bk(e2));
			 e2++;
		 }
